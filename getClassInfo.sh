...
#Add if non-simplified version
if [ $3 = "True" ]
then
	mv tempRefined.txt $1$2.txt
else
	echo $1$2 > tempRefined.txt
	mv tempRefined.txt $1$2.txt
fi
...
then
	...
	echo $1$2NextSem > $1$2NextSem.txt
fi

#Check if course exists next semester
if [ ! -s $1$2NextSem.txt ]
then
    echo "Course '$1 $2' doesn't exist or is not offered in $sem"
    rm $1$2NextSem.txt
	...
	then
		...
		do
   		:
			...
			if [ $4 = "False" ] || [ ${#myTest} -gt 14 ] || [ -z $email ]
			then
				rm temp.txt
			else
				cat $1$2.txt | sed "s/$lastName $firstInit/$firstInit $lastName\n Instructor's Email: $email/g" > temp.txt
				rm $1$2.txt
				mv temp.txt $1$2.txt
			fi
		done
...
#Output final results if they exist 
if [ -f $1$2.txt ]
then

	if  [ $5 = "True" ]
	then
		cat $1$2.txt | sed "s/Prerequisites:.*//g" > preqtemp.txt
		cat preqtemp.txt > $1$2.txt
		rm preqtemp.txt
	fi
	cat $1$2.txt
fi
